...
$query = "select `type`, sum(amount) as sum_amount from payment_histories where payment_method_id"
    . " in (select `id` from payment_methods where `type`='" . PAYMENT_METHOD_BANK . "')"
    . " group by `type`";
...
<div class="tile-stats">
  ...
  <?php if ($show_chart): ?>
    ...
    <div id="total_bank_pie_chart" style="height: 300px;"></div>

    <script>
      $(function () {
        var total_bank_pie_chart = echarts.init(document.getElementById('total_bank_pie_chart'), {
          color: ['#26B99A', '#E74C3C']
        });
        total_bank_pie_chart.setOption({
          tooltip: {
            trigger: 'item',
