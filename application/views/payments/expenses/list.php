...
<div class="page-title">
  ...
  <div class="title_right">
    <div class="pull-right">
      <button type="button" class="btn btn-round btn-primary btn-sm" onclick="location.href = '<?php echo base_url("payments/expenses/add"); ?>/' + document.getElementById('expenses_type_id').value"><?php ___("Add New Expenses"); ?></button>
    </div>
  </div>
</div>
      ...
      <div class="x_content">
        <div class="row">
          <form method="post" name="frmFilter" id="frmFilter" class="form-filters">
            <label for="expenses_type_id"><?php ___("Type"); ?>:</label>
            <select name="expenses_type_id" id="expenses_type_id" onchange="changeExpensesType(this.value)" class="form-control">
              <option value="all">- <?php ___("All"); ?> -</option>
              <?php foreach ($this->expenses_types as $id => $name): ?>
                <option value="<?php echo $id; ?>"><?php echo $name; ?></option>
              <?php endforeach; ?>
            </select>
            
            <div id="company-filter" style="display: none;">
        ...
        <table id="dataTable" class="table table-striped table-bordered" width="100%">
          <thead>
            <tr>
              ...
              <th width="100"><?php ___("Payment") ?>:</th>
              <th width="120"><?php ___("Amount(%s)", my_get_currency_unit()) ?>:</th>
...
<script>
  ...
  var expenses_type = '<?php echo $expenses_type; ?>';
      ...
      ajax: {
        ...
        data: function (d) {
          lastSearchParams = $.extend(lastSearchParams, d, {
            expenses_type: document.frmFilter.expenses_type_id.value,
...
//          console.log(lastSearchParams);
    ...
    if (expenses_type != 'all') {
      $("#expenses_type_id").val(expenses_type);
      changeExpensesType(expenses_type);
    }
  });
  ...
  function finish_payment(id) {
    if (confirm("<?php ___("Are you sure finish selected payment history?"); ?>")) {
      $.get("<?php echo base_url('payments/expenses/ajax_finish') ?>/" + id, function () {
        reloadTable(false);
      })
    }
  }

  function changeExpensesType(type) {
    if (type == '<?php echo PAYMENT_REASON_TYPE_EXPENSES; ?>') {
      $("#company-filter").show();
  ...
  function exportExcel() {
    ...
    url += "?expenses_type=" + lastSearchParams.expenses_type;
