      ...
      <div class="x_content">
        <div class="item form-group">
          ...
          <div class="col-md-9 col-sm-8 col-xs-12 control-input">
            <input type="text" id="em-pay_date" name="pay_date" value="<?php echo ($this->payment_history_m->pay_date ? $this->payment_history_m->pay_date : date('Y-m-d')); ?>" required="required" class="form-control date-picker" autocomplete="off" size="10" style="width:100px; float:left; margin-right:20px;">
            <?php
...
<script>
  $(function () {
    ...
    $("#em-category_id").change(function () {
      $.getJSON("<?php echo site_url("payments/expenses/ajax_get_procuts") ?>/" + $(this).val(), function (products) {
        var options = '';
...
<?php endif; ?>
  ...
  function get_product_price(product_id) {
    if (product_id == 0) {
      ...
      $.get("<?php echo site_url("payments/expenses/ajax_get_procut_price") ?>/" + $("#em-product_id").val(), function (price) {
        $("#em-amount").val(price);
      });
    }
  }
