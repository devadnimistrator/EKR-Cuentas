...
<script>
  function changeFilter() {
    location.href = "<?php echo base_url("tangible/products./index"); ?>/" + $("#filter_category").val();
  }
  ...
  $(document).ready(function () {
    $tableProducts = $('#table-products').DataTable({
      ...
      serverSide: true,
      ajax: {
        url: "<?php echo base_url("tangible/products./ajax_find/" . $category); ?>",
        type: "POST",
        data: function (d) {
          return $.extend({}, d, {
      ...
      createdRow: function (row, data, index) {
        ...
        $(row).dblclick(function () {
          location.href = '<?php echo base_url('tangible/products./edit'); ?>/' + $(this).attr('id');
        });
      },
    });
  });
  ...
  function delete_product(id) {
    if (confirm("<?php ___("Are you sure delete selected product?"); ?>")) {
      $.get("<?php echo base_url('tangible/products/ajax_delete') ?>/" + id, function () {
        reloadTable(false);
      })
    }
  }
