...
class Education_product_m extends My_Model {
  ...
  private function set_filter($category, $search) {
    ...
    if ($category != 'all') {
      ...
      $this->db->or_where("category_id in (select `id` from education_categories where parent_id =" . $category . ')');
      $this->db->group_end();
    }
  }
  ...
  public function get_products($category_id = 0, $index_array = true) {
    if ($category_id > 0) {
      ...
      $this->db->or_where("category_id in (select `id` from education_categories where parent_id=" . $category_id . ")");
    }
